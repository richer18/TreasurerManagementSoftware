...
function GenerateReport({ open, onClose }) {
  ...
  const [ctcnFrom, setCtcnFrom] = useState('');
  const [ctcnTo, setCtcnTo] = useState('');
...
const cashierOptionsByReport = {
  ...
  real_property_tax_data: ['RICARDO ENOPIA', 'IRIS RAFALES', 'FLORA MY FERRER'],
...
{/* Conditionally render either CTC or OR filters */}
{(reportType === 'real_property_tax_data' || reportType === '51') && cashier ? (
  <>
    <Grid item xs={12} md={6}>
      <TextField
        fullWidth
  ...
  <>
    <Grid item xs={12} md={6}>
      <TextField
        ...
        type="number"
        value={ctcnFrom}
        onChange={(e) => setCtcnFrom(e.target.value)}
        size="small"
        variant="outlined"
      />
    </Grid>
    <Grid item xs={12} md={6}>
      <TextField
        ...
        type="number"
        value={ctcnTo}
        onChange={(e) => setCtcnTo(e.target.value)}
        size="small"
        variant="outlined"
      />
    </Grid>
