...
export default function Esre() {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        setRegulatoryFeesData(prev => ({ ...prev, loading: true, error: null }));
        setServiceUserChargesData(prev => ({ ...prev, loading: true, error: null }));
        setReceiptsEconomicEntDataData(prev => ({ ...prev, loading: true, error: null }));
        setOtherTaxesData(prev => ({ ...prev, loading: true, error: null }));
  
        const [taxRes, regRes, serRes, reeRes,otRes] = await Promise.all([
          fetch(`http://192.168.101.108:3001/api/TaxOnBusinessTotalESREBox?${params}`),
          fetch(`http://192.168.101.108:3001/api/RegulatoryFeesAndChargesTotalESREBox?${params}`),
          fetch(`http://192.168.101.108:3001/api/ServiceUserChargesTotalESREBox?${params}`),
          fetch(`http://192.168.101.108:3001/api/ReceiptsEconomicEnterprisesTotalESREBox?${params}`),
          fetch(`http://192.168.101.108:3001/api/OtherTaxesTotalESREBox?${params}`)
        ]);
        ...
        const [taxData, regData, serData,reeData, otsData] = await Promise.all([
          checkJson(taxRes),
          checkJson(regRes),
          checkJson(serRes),
          checkJson(reeRes),
        ...
        if (typeof taxData.total === 'undefined' || typeof taxData.currency === 'undefined') {
          throw new Error('Invalid structure in tax data');
        }
  
        if (typeof regData.total === 'undefined' || typeof regData.currency === 'undefined') {
          throw new Error('Invalid structure in regulatory data');
        }
        if (typeof serData.total === 'undefined' || typeof serData.currency === 'undefined') {
          throw new Error('Invalid structure in regulatory data');
        }
        if (typeof reeData.total === 'undefined' || typeof reeData.currency === 'undefined') {
          throw new Error('Invalid structure in regulatory data');
        }
        if (typeof otsData.total === 'undefined' || typeof otsData.currency === 'undefined') {
          throw new Error('Invalid structure in regulatory data');
        }
  
        setTaxBusinessData({ value: taxData.total, loading: false, error: null });
        setRegulatoryFeesData({ value: regData.total, loading: false, error: null });
        setServiceUserChargesData({ value: serData.total, loading: false, error: null });
        setReceiptsEconomicEntDataData({ value: reeData.total, loading: false, error: null });
        setOtherTaxesData({ value: otsData.total, loading: false, error: null });
  
      } catch (err) {
        console.error('Fetch error:', err);
  
        setTaxBusinessData({ value: 0, loading: false, error: `Data load failed: ${err.message}` });
        setRegulatoryFeesData({ value: 0, loading: false, error: `Data load failed: ${err.message}` });
        setServiceUserChargesData({ value: 0, loading: false, error: `Data load failed: ${err.message}` });
        setReceiptsEconomicEntDataData({ value: 0, loading: false, error: `Data load failed: ${err.message}` });
        setOtherTaxesData({ value: 0, loading: false, error: `Data load failed: ${err.message}` });
      }
    };
  ...
  <Grid container spacing={2}>
    <Grid item xs={12} sm={6}>
      ...
      <Menu anchorEl={anchorQuarter} open={Boolean(anchorQuarter)} onClose={() => setAnchorQuarter(null)}>
        {quarterOptions.map((quarter) => (
          <MenuItem key={quarter} onClick={() => {
    ...
    <Grid item xs={12} sm={6}>
      ...
      <Menu anchorEl={anchorYear} open={Boolean(anchorYear)} onClose={() => setAnchorYear(null)}>
    ...
    {/* Dialog Section */}
    <Dialog open={Boolean(selectedBox)} onClose={handleDialogClose} maxWidth="sm" fullWidth>
      <DialogTitle sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <Typography variant="h6">{selectedBox?.title || 'Details'}</Typography>
